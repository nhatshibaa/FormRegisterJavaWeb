    ...
    public List<Customer> findAll() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                LocalDateTime dob =
                        LocalDateTime.ofInstant(resultSet.getTimestamp("dateOfBirth").toInstant(), ZoneId.systemDefault());
                ...
                Customer customer = new Customer(id, name, phone, avatar, dob, createdAt, updatedAt, status);
                list.add(customer);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                LocalDateTime updatedAt =
                        LocalDateTime.ofInstant(resultSet.getTimestamp("updatedAt").toInstant(), ZoneId.systemDefault());
                int status = resultSet.getInt("status");
                customer = new Customer(id, name, phone, avatar, dob, createdAt, updatedAt, status);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Customer update(int id, Customer updateCustomer) {
        try {
            ...
            String sqlQuery = "update customers " +
                    "set id = ?, name = ?, phone = ?, image = ?, dateOfBirth = ?, createdAt = ?, updatedAt = ?, status = ? where id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
            ...
            preparedStatement.setString(5, updateCustomer.getDateOfBirth().toString());
